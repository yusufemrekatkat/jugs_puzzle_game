		...
		for(action=0; action<ACTIONS_NUMBER; action++)
		{
			...
			if(child!=NULL) {
				...
				case GeneralizedAStarSearch:
					if (temp_node != NULL) {
						// Calculate f(n) = g(n) + N1 * h(n) for both child and temp_node
						float child_fn = child->path_cost + alpha * Compute_Heuristic_Function(&(child->state), goal_state);
						float temp_fn = temp_node->path_cost + alpha * Compute_Heuristic_Function(&(temp_node->state), goal_state);

						if (child_fn < temp_fn) {
							// Child is better, remove the old node
							Remove_Node_From_Frontier(temp_node, &frontier);
						} else {
							// Existing node is better, skip this child
							continue;
						}
					}
					// Compute heuristic for the child
					child->state.h_n = Compute_Heuristic_Function(&(child->state), goal_state);
					// Insert into frontier using Generalized A* priority
...
void Insert_Priority_Queue_GENERALIZED_A_Star(Node *const child, Queue **frontier, float alpha)
{
	...
	// Compute f(n) = g(n) + N1 * h(n) for the child
	float child_fn = child->path_cost + alpha * child->state.h_n;
	float first_fn = (*frontier)->node->path_cost + alpha * (*frontier)->node->state.h_n;

	// If child has lower f(n) than the first node in frontier
	if(child_fn < first_fn) {
		new_queue->next = *frontier;
		*frontier = new_queue;
		return;
	}

	// Find the correct position in the frontier
	temp_queue = *frontier;
	while(temp_queue->next != NULL) {
		float next_fn = temp_queue->next->node->path_cost + alpha * temp_queue->next->node->state.h_n;
		if(child_fn < next_fn) {
			new_queue->next = temp_queue->next;
			temp_queue->next = new_queue;
			return;
	...
	// If child has the highest f(n), add it at the end
	temp_queue->next = new_queue;
	new_queue->next = NULL;
}
//______________________________________________________________________________
