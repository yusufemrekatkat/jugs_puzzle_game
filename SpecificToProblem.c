...
int Result(const State *const parent_state, const enum ACTIONS action, Transition_Model *const trans_model)
{
    ...
    int jug_to_fill, jug_to_empty, jug_from, jug_to;

    switch(action) {
        case FILL_JUG_0:
            jug_to_fill = 0;
            if (temp_state.jug_levels[jug_to_fill]==JUG_CAPACITIES[jug_to_fill]){
                return 0;
            }
            temp_state.jug_levels[jug_to_fill]=JUG_CAPACITIES[jug_to_fill];
            break;
